<template>
  <main class="work-content">
    <h1 class="page-title">Work</h1>

    <div class="projects-container">
      <article class="project-card">
        <img class="project-image" src="@/assets/img/orisha.png">
        <div class="project-info">
          <h2 class="project-title">Healthcare Billing Engine</h2>
          <p class="project-description">
            Cloud-based billing platform for healthcare professionals in France.
          </p>
          <div class="project-tags">
            <span class="tag">ASP.NET</span>
            <span class="tag">VB.NET</span>
            <span class="tag">C#</span>
            <span class="tag">SQL</span>
            <span class="tag">Oracle</span>
            <span class="tag">REST</span>
            <span class="tag">SOAP</span>
            <span class="tag">Docker</span>
          </div>
        </div>
      </article>

      <article class="project-card">
        <img class="project-image" src="@/assets/img/orisha.png">
        <div class="project-info">
          <h2 class="project-title">Smart Card Engine</h2>
          <p class="project-description">
            Cross-platform engine to read health cards and manage secure devices.
          </p>
          <div class="project-tags">
            <span class="tag">Go</span>
            <span class="tag">Vue.js</span>
            <span class="tag">TypeScript</span>
            <span class="tag">Wails</span>
            <span class="tag">API</span>
            <span class="tag">Cross-Platform</span>
          </div>
        </div>
      </article>

      <article class="project-card">
        <img class="project-image" src="@/assets/img/orisha.png">
        <div class="project-info">
          <h2 class="project-title">FSE Dispatch</h2>
          <p class="project-description">
            Automation tool for executing processes and API commands.
          </p>
          <div class="project-tags">
            <span class="tag">Go</span>
            <span class="tag">REST API</span>
            <span class="tag">Logger</span>
            <span class="tag">Automation</span>
            <span class="tag">Error Handling</span>
          </div>
        </div>
      </article>

      <article class="project-card">
        <img class="project-image" src="@/assets/img/orisha.png">
        <div class="project-info">
          <h2 class="project-title">PyxCloud</h2>
          <p class="project-description">
            Web-based billing platform with tenant and instance management.
          </p>
          <div class="project-tags">
            <span class="tag">PHP</span>
            <span class="tag">Symfony</span>
            <span class="tag">PostgreSQL</span>
            <span class="tag">Vue.js</span>
            <span class="tag">Kubernetes</span>
            <span class="tag">Filtering Engine</span>
          </div>
        </div>
      </article>

      <article class="project-card">
        <img class="project-image" src="@/assets/img/github.webp">
        <div class="project-info">
          <h2 class="project-title">Terminal</h2>
          <p class="project-description">
            A simple terminal emulator built to explore Rust fundamentals.
          </p>
          <div class="project-tags">
            <span class="tag">Rust</span>
            <span class="tag">CLI</span>
            <span class="tag">Personal Project</span>
          </div>
        </div>
      </article>

      <article class="project-card">
        <img class="project-image" src="@/assets/img/github.webp">
        <div class="project-info">
          <h2 class="project-title">MovieShowTracker</h2>
          <p class="project-description">
            Android app to track movies and shows, inspired by Letterboxd.
          </p>
          <div class="project-tags">
            <span class="tag">Kotlin</span>
            <span class="tag">Android</span>
            <span class="tag">IMDB API</span>
            <span class="tag">Watchlist</span>
          </div>
        </div>
      </article>

      <article class="project-card">
        <img class="project-image" src="@/assets/img/orisha.png">
        <div class="project-info">
          <h2 class="project-title">FileManager</h2>
          <p class="project-description">
            Cloud file management tool for listing, editing, and organizing files.
          </p>
          <div class="project-tags">
            <span class="tag">PHP</span>
            <span class="tag">Vue.js</span>
            <span class="tag">Kubernetes</span>
            <span class="tag">REST API</span>
          </div>
        </div>
      </article>
    </div>
  </main>
</template>